...
class WaffleCanastaNieveExistenciasController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const query = 'SELECT * FROM wcne WHERE sucursal_id = ? AND adminId = ?';
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const nieveId = req.params.id; // ID del juego solicitado
                    const query = 'SELECT * FROM wcne WHERE product_id = ? AND sucursal_id = ? AND adminId = ?';
                    const bebidas = yield database_1.default.promise().query(query, [nieveId, sucursalId, adminId])
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        const updateNieve = req.body.existencia;
                        yield database_1.default.promise().query('UPDATE wcne SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateNieve, nieveId, sucursalId, adminId])
                        ...
                        yield database_1.default.promise().query('UPDATE wcne SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateNieve, inventario, nieveId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
const waffleCanastaNEController = new WaffleCanastaNieveExistenciasController();
exports.default = waffleCanastaNEController;
