...
class WaffleCanastaDecoracionExistenciasController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const query = 'SELECT * FROM wcde WHERE sucursal_id = ? AND adminId = ?';
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const query = 'SELECT * FROM wcde WHERE product_id = ? AND sucursal_id = ? AND adminId = ?';
                    const bebidas = yield database_1.default.promise().query(query, [decoracionId, sucursalId, adminId])
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE wcde SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updatedecoracion, decoracionId, sucursalId, adminId])
                        ...
                        yield database_1.default.promise().query('UPDATE wcde SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updatedecoracion, inventario, decoracionId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
const waffleCanastaDEController = new WaffleCanastaDecoracionExistenciasController();
exports.default = waffleCanastaDEController;
