...
class WaffleCanastaIngredienteUntableExistenciasController {
    list(req, res) {
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE wciue SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, ingredienteId, sucursalId, adminId])
                            .then(() => {
                        ...
                        yield database_1.default.promise().query('UPDATE wciue SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, inventario, ingredienteId, sucursalId, adminId])
                            .then(() => {
                            res.json({ message: "El stock fue actualizado" });
                        })
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
...
const waffleCanastaIUEController = new WaffleCanastaIngredienteUntableExistenciasController();
exports.default = waffleCanastaIUEController;
