            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE wcice SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, ingredienteId, sucursalId, adminId])
                            .then(() => {
                        ...
                        yield database_1.default.promise().query('UPDATE wcice SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, inventario, ingredienteId, sucursalId, adminId])
                            .then(() => {
                            res.json({ message: "El stock fue actualizado" });
                        })
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
...
exports.default = waffleCanastaICEController;
