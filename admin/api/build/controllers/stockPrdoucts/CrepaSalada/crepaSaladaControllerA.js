...
class CrepaSaladaAderesoExistenciasController {
    list(req, res) {
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE csae SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateAdereso, aderesoId, sucursalId, adminId])
                        ...
                        yield database_1.default.promise().query('UPDATE csae SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateAdereso, inventario, aderesoId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
const crepaSaladaAEController = new CrepaSaladaAderesoExistenciasController();
exports.default = crepaSaladaAEController;
