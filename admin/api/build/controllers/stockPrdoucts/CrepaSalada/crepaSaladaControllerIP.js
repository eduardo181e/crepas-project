            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE csipe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, ingredienteId, sucursalId, adminId])
                        ...
                        yield database_1.default.promise().query('UPDATE csipe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, inventario, ingredienteId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
exports.default = crepaSaladaIPEController;
