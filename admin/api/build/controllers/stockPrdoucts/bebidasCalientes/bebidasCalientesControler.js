...
class BebidasCalientesExistenciasController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const bebidas = yield database_1.default.promise().query(query, [sucursalId, adminId])
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const bebidas = yield database_1.default.promise().query(query, [bebidaId, sucursalId, adminId])
                        .then(bebidas => bebidas[0])
                        .catch(err => console.log(err));
                    if (bebidas.length > 0) {
                        res.json(bebidas);
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE bce SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, bebidaId, sucursalId, adminId])
                            .then(updtae => {
                            console.log(updtae);
                        ...
                        console.log(bebidaId);
                        console.log(sucursalId);
                        yield database_1.default.promise().query('UPDATE bce SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, inventario, bebidaId, sucursalId, adminId])
                            .then(updtae => {
                            console.log(updtae);
                            res.json({ message: 'El stock fue actualizado' });
                        })
...
const bebidasCalientesExistenciasController = new BebidasCalientesExistenciasController();
exports.default = bebidasCalientesExistenciasController;
