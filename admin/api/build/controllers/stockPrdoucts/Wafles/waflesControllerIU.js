...
class WaffleIngredienteUntableExistenciasController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const query = 'SELECT * FROM wiue WHERE sucursal_id = ? AND adminId = ?';
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE wiue SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, ingredienteId, sucursalId, adminId])
                            .then(() => {
                        ...
                        yield database_1.default.promise().query('UPDATE wiue SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, inventario, ingredienteId, sucursalId, adminId])
                            .then(() => {
                            res.json({ message: "El stock fue actualizado" });
                        })
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
...
const waffleIUEController = new WaffleIngredienteUntableExistenciasController();
exports.default = waffleIUEController;
