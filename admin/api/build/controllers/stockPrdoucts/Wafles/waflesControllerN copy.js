...
class WaffleNieveExistenciasController {
    list(req, res) {
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE wne SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateNieve, nieveId, sucursalId, adminId])
                        ...
                        yield database_1.default.promise().query('UPDATE wne SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateNieve, inventario, nieveId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
const waffleNEController = new WaffleNieveExistenciasController();
exports.default = waffleNEController;
