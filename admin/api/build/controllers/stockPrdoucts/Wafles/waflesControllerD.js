            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE wde SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updatedecoracion, decoracionId, sucursalId, adminId])
                        ...
                        yield database_1.default.promise().query('UPDATE wde SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updatedecoracion, inventario, decoracionId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
exports.default = waffleDEController;
