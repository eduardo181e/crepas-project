            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        yield database_1.default.promise().query('UPDATE bfe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, bebidaId, sucursalId, adminId])
                            .then(updtae => {
                            console.log(updtae);
                        ...
                        console.log(bebidaId);
                        console.log(sucursalId);
                        yield database_1.default.promise().query('UPDATE bfe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, inventario, bebidaId, sucursalId, adminId])
                            .then(updtae => {
                            console.log(updtae);
                            res.json({ message: 'El stock fue actualizado' });
                        })
