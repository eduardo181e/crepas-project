...
class CrepaDulceIngredienteHarinaExistenciasController {
    list(req, res) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    const harinaId = req.params.id; // ID del juego solicitado
                    const query = 'SELECT * FROM cdthe WHERE product_id = ? AND sucursal_id = ? AND adminId = ?';
                    const bebidas = yield database_1.default.promise().query(query, [harinaId, sucursalId, adminId])
            ...
            if (req.headers['authorization'] === undefined) {
                ...
                try {
                    ...
                    if (req.body.cantidad === 0) {
                        ...
                        const updateHarina = req.body.existencia;
                        yield database_1.default.promise().query('UPDATE cdthe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateHarina, harinaId, sucursalId, adminId])
                        ...
                        const harinaId = req.params.id;
                        const updateHarina = req.body.existencia;
                        const updateInventario = req.body.inventario;
                        yield database_1.default.promise().query('UPDATE cdthe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateHarina, updateInventario, harinaId, sucursalId, adminId])
                            .then(() => res.json({ message: 'El stock fue actualizado' }))
                            .catch(err => res.json({ 'Error al actualizar el stock:': err }));
                    }
                }
...
const crepaDulceHEController = new CrepaDulceIngredienteHarinaExistenciasController();
exports.default = crepaDulceHEController;
