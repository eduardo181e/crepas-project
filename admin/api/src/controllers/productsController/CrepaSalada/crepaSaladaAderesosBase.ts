...
class CrepaSaladaAderesoBaseController {
    ...
    public async create(req: Request, res: Response): Promise<void> {
        if(req.headers['authorization'] === undefined){
         ...
         try{
          ...
          await pool.promise().query('INSERT INTO csab SET ?', {adereso_base:adereso.trim(), inventario: req.body.inventario, adminId: adminId})
          .then((result:any) => {
          ...
          .then(async (sucursales:any) => {
            sucursales[0].forEach(async (sucursal:any) => {
              ...
              await pool.promise().query('INSERT INTO csabe SET ?', [registro])
              .then(() => {
    ...
    public async delete(req: Request, res: Response){
        ...
        await pool.promise().query(query, [aderesoId, adminId])
            ...
            await pool.promise().query('DELETE FROM csabe WHERE product_id = ? AND adminId = ?', [aderesoId, adminId])
            .then(() => {
    ...
    public async update(req: Request, res: Response){
        if(req.headers['authorization'] === undefined){
            ...
            const adminId = decodedToken.id
              ...
              await pool.promise().query(query, [{adereso_base:req.body.adereso_base.trim()}, aderesoId, adminId])
            .then(() => {
            ...
            await pool.promise().query('UPDATE csabe SET ? WHERE product_id = ? AND adminId = ?', [{adereso_base:req.body.adereso_base.trim()}, aderesoId, adminId])
            .then(() => {
            ...
            .catch(err => {
              ...
              const query = 'UPDATE csab SET ? WHERE id = ? AND adminId = ?';
              await pool.promise().query(query, [{adereso_base:req.body.adereso_base.trim() ,inventario: parseInt(inventario)}, aderesoId, adminId])
            .then(() => {
                res.json({ message: 'adereso actualizado' });
            })
            .catch(err => {
            ...
            // Actualizar registro de existencias y ventas
            await pool.promise().query('UPDATE csabe SET ? WHERE product_id = ? AND adminId = ?', [{adereso_base:req.body.adereso_base.trim(), cantidad: parseInt(inventario), inventario: 0, existencia:0}, aderesoId, adminId])
            .then(() => {
              console.log('registro de existencias actualizado');
            })
            .catch(err => {
