...
class WaflesIngredienteUntableController {
  public async list(req: Request, res: Response): Promise<void> {
    ...
    const query = 'SELECT * FROM wiu WHERE adminId = ?';
...
public async getOne(req: Request, res: Response){
    ...
    const query = 'SELECT * FROM wiu WHERE id = ? AND adminId = ?';
...
public async create(req: Request, res: Response): Promise<void> {
    if(req.headers['authorization'] === undefined){
      ...
      await pool.promise().query('INSERT INTO wiu SET ?', {ingrediente_unt:ingrediente, inventario: req.body.inventario, adminId: adminId})
      .then((result:any) => {
    ...
    .then ((sucursales:any) => {
      sucursales[0].forEach(async (sucursal:any) => {
        ...
        await pool.promise().query('INSERT INTO wiue SET ?', [registro])
        .then(() => {
...
public async delete(req: Request, res: Response){
    ...
    const query = 'DELETE FROM wiu WHERE id = ? AND adminId = ?';
    ...
    await pool.promise().query('DELETE FROM wiue WHERE product_id = ? AND adminId = ?', [ingredienteId, adminId])
    .then(() => {
...
public async update(req: Request, res: Response){
  if(req.headers['authorization'] === undefined){
    ...
    const adminId = decodedToken.id
      ...
      const query = 'UPDATE wiu SET ? WHERE id = ? AND  adminId = ?';
      ...
      await pool.promise().query('UPDATE wiue SET ? WHERE product_id = ? AND adminId = ?', [{ingrediente_unt: req.body.ingrediente_unt}, ingredienteId, adminId])
      .then(() => {
      ...
      .catch(err => {
        ...
        await pool.promise().query('UPDATE wiue SET ? WHERE product_id = ? AND adminId = ?', [{ingrediente_unt: req.body.ingrediente_unt, cantidad: inventario, inventario: 0, existencia: 0}, ingredienteId, adminId])
        .then(() => {
          console.log('registro de existencias actualizado')
        })
        .catch(err => {
...
export default waflesIUController;