...
public async create(req: Request, res: Response): Promise<void> {
    if(req.headers['authorization'] === undefined){
      ...
      await pool.promise().query('INSERT INTO wcn SET ?', {nieve:nieve, inventario: req.body.inventario, adminId: adminId})
      .then((result:any) => {
      ...
      .then ((sucursales:any) => {
        sucursales[0].forEach(async (sucursal:any) => {
          ...
          await pool.promise().query('INSERT INTO wcne SET ?', [registro])
          .then(() => {
...
if(req.headers['authorization'] === undefined){
    ...
    await pool.promise().query('DELETE FROM wcne WHERE product_id = ? AND adminId', [nieveId, adminId])
    .then(() => {
...
public async update(req: Request, res: Response){
    ...
    await pool.promise().query(query, [{nieve:req.body.nieve}, nieveId, adminId])
      ...
      await pool.promise().query('UPDATE wcne SET ? WHERE product_id = ? AND adminId = ?', [{nieve:req.body.nieve}, nieveId, adminId])
      .then(() => {
      ...
      .catch(err => {
        ...
        await pool.promise().query(query, [{nieve:req.body.nieve, inventario: inventario}, nieveId, adminId])
          ...
          // Actualizar registros de venas y existencias de nieves
          await pool.promise().query('UPDATE wcne SET ? WHERE product_id = ? AND adminId = ?', [{nieve:req.body.nieve, cantidad: inventario, inventario: 0, existencia:0}, nieveId, adminId])
          .then(() => {
            console.log('registro de existencias actualizado')
          })
          .catch(err => {
...
export default waflesCanastaNController;