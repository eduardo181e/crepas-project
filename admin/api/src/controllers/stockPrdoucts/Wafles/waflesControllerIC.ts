...
await pool.promise().query('UPDATE wice SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, ingredienteId, sucursalId, adminId])
.then(() => {
...
.catch(err => res.json({'Error al actualizar el stock:':err}));}else if(req.body.cantidad === 1){
  ...
  await pool.promise().query('UPDATE wice SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBebida, inventario, ingredienteId, sucursalId, adminId])
  .then(() => {
    res.json({ message: "El stock fue actualizado" })
  })
...
export default waffleICEController;