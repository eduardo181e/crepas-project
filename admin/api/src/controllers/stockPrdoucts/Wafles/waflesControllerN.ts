...
await pool.promise().query('UPDATE wne SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateNieve, nieveId, sucursalId, adminId])
...
.catch(err => res.json({'Error al actualizar el stock:':err}));
        ...
        await pool.promise().query('UPDATE wne SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateNieve, inventario, nieveId, sucursalId, adminId])
        .then(() => res.json({ message: 'El stock fue actualizado' }))
        .catch(err => res.json({'Error al actualizar el stock:':err}));
      }
  }catch (error:any) {
...
export default waffleNEController;