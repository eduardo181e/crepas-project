...
class CrepaDulceIngredienteHarinaExistenciasController {
    ...
    const bebidas: any = await pool.promise().query(query, [harinaId, sucursalId, adminId])
    ...
    await pool.promise().query('UPDATE cdthe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateHarina, harinaId, sucursalId, adminId])
    ...
    .catch(err => res.json({'Error al actualizar el stock:':err}));}else if(req.body.cantidad === 1){
      ...
      await pool.promise().query('UPDATE cdthe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateHarina, updateInventario, harinaId, sucursalId, adminId])
      .then(() => res.json({message: 'El stock fue actualizado'}))
...
export default crepaDulceHEController;