...
await pool.promise().query('UPDATE wcde SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updatedecoracion, decoracionId, sucursalId, adminId])
...
.catch(err => res.json({'Error al actualizar el stock:':err}));
        ...
        await pool.promise().query('UPDATE wcde SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updatedecoracion, inventario, decoracionId, sucursalId, adminId])
        .then(() => res.json({ message: 'El stock fue actualizado' }))
        .catch(err => res.json({'Error al actualizar el stock:':err}));
      }
  }catch (error:any) {
...
export default waffleCanastaDEController;