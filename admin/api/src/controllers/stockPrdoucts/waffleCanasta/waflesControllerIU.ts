...
await pool.promise().query('UPDATE wciue SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, ingredienteId, sucursalId, adminId])
    .then(() => {
    ...
    .catch(err => res.json({'Error al actualizar el stock:':err}));
      ...
      await pool.promise().query('UPDATE wciue SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, inventario, ingredienteId, sucursalId, adminId])
          .then(() => {
              res.json({ message: "El stock fue actualizado" })
          })    
          .catch(err => res.json({'Error al actualizar el stock:':err}));
...
export default waffleCanastaIUEController;