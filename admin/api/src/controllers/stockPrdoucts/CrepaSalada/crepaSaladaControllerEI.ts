...
class CrepaSaladaEnsaladaIndividualExistenciasController {
    ...
    const bebidas: any = await pool.promise().query(query, [ensaladaId, sucursalId, adminId])
    ...
    if(cantidad === 0){
    const sucursalId = req.body.sucursal_id;
    const ensaladaId = req.params.id;
    const updateEnsalada = req.body.existencia;
    await pool.promise().query('UPDATE cseie SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateEnsalada, ensaladaId, sucursalId, adminId])
    .then(() => res.json({ message: 'El stock fue actualizado' }))
    .catch(err => res.json({'Error al actualizar el stock:':err}));}else if(cantidad === 1){
      const sucursalId = req.body.sucursal_id;
      const ensaladaId = req.params.id;
      const updateEnsalada = req.body.existencia;
      const inventario = req.body.inventario;
      await pool.promise().query('UPDATE cseie SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateEnsalada, inventario, ensaladaId, sucursalId, adminId])
      .then(() => res.json({ message: 'El stock fue actualizado' }))
...
const crepaSaladaEIEController = new CrepaSaladaEnsaladaIndividualExistenciasController();
export default crepaSaladaEIEController;