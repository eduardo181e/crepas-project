...
class CrepaSaladaBotanaExistenciasController {
    ...
    const bebidas: any = await pool.promise().query(query, [botanaId, sucursalId, adminId])
    ...
    await pool.promise().query('UPDATE csbe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBotana, botanaId, sucursalId, adminId])
    ...
    .catch(err => res.json({'Error al actualizar el stock:':err}));
            ...
            await pool.promise().query('UPDATE csbe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateBotana, inventario, botanaId, sucursalId, adminId])
            .then(() => res.json({ message: 'El stock fue actualizado' }))
            .catch(err => res.json({'Error al actualizar el stock:':err}));
          }
      }catch (error:any) {
...
export default crepaSaladaBEController;