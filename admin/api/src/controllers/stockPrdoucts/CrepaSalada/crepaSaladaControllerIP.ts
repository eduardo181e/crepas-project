...
class CrepaSaladaIngredientePrincipalExistenciasController {
    public async list(req: Request, res: Response): Promise<void> {
        ...
        const query = 'SELECT * FROM csipe WHERE sucursal_id = ? AND adminId = ?';
    ...
    await pool.promise().query('UPDATE csipe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, ingredienteId, sucursalId, adminId])
    ...
    .catch(err => res.json({'Error al actualizar el stock:':err}));}else if(cantidad === 1){
      ...
      await pool.promise().query('UPDATE csipe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, inventario, ingredienteId, sucursalId, adminId])
      .then(() => res.json({ message: 'El stock fue actualizado' }))
...
const crepaSaladaIPEController = new CrepaSaladaIngredientePrincipalExistenciasController();
export default crepaSaladaIPEController;