...
class CrepaSaladaIngredienteBaseExistenciasController {
    public async list(req: Request, res: Response): Promise<void> {
        ...
        const query = 'SELECT * FROM csibe WHERE sucursal_id = ? AND adminId = ?';
    ...
    await pool.promise().query('UPDATE csibe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, ingredienteId, sucursalId, adminId])
    ...
    await pool.promise().query('UPDATE csibe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateIngrediente, inventario, ingredienteId, sucursalId, adminId])
    .then(() => res.json({ message: 'El stock fue actualizado' }))
...
const crepaSaladaIBEController = new CrepaSaladaIngredienteBaseExistenciasController();
export default crepaSaladaIBEController;