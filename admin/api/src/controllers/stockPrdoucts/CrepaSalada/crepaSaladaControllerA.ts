...
class CrepaSaladaAderesoExistenciasController {
    ...
    await pool.promise().query('UPDATE csae SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateAdereso, aderesoId, sucursalId, adminId])
    ...
    await pool.promise().query('UPDATE csae SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateAdereso, inventario, aderesoId, sucursalId, adminId])
    .then(() => res.json({ message: 'El stock fue actualizado' }))
    .catch(err => res.json({'Error al actualizar el stock:':err}));
    }
      }catch (error:any) {
...
export default crepaSaladaAEController;