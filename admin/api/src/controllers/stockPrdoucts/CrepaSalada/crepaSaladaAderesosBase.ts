...
class CrepaSaladaAderesoBaseExistenciasController {
    ...
    const bebidas: any = await pool.promise().query(query, [aderesoId, sucursalId, adminId])
    ...
    await pool.promise().query('UPDATE csabe SET existencia = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateAdereso, aderesoId, sucursalId, adminId])
    ...
    .catch(err => res.json({'Error al actualizar el stock:':err}));
            ...
            await pool.promise().query('UPDATE csabe SET existencia = ?, inventario = ? WHERE product_id = ? AND sucursal_id = ? AND adminId = ?', [updateAdereso, inventario, aderesoId, sucursalId, adminId])
            .then(() => res.json({ message: 'El stock fue actualizado' }))
            .catch(err => res.json({'Error al actualizar el stock:':err}));
        }else if(cantidad === 1){
          const sucursalId = req.body.sucursal_id;
...
export default crepaSaladaABEController;