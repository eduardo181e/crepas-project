...
import { BebidasCalientesStockComponent } from './bebidas-calientes-stock.component';
...
describe('BebidasCalientesStockComponent', () => {
  let component: BebidasCalientesStockComponent;
  let fixture: ComponentFixture<BebidasCalientesStockComponent>;
  ...
  let BebidasCalientesStockServiceMock = {
    getBebidasCalientes: jasmine.createSpy('getBebidasCalientes').and.returnValue(of(mockBebidas)),
    updateStockBebidaCaliente: jasmine.createSpy('updateStockBebidaCaliente').and.returnValue(of(mockBebidasExistencia))
  }
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [ BebidasCalientesStockComponent, TranslatePipe ],
      providers: [
        { provide: AlertDialogService, useValue: alertService },
        { provide: Router, useValue: mockRouter },
        { provide: BebidasCalientesStockService, useValue: BebidasCalientesStockServiceMock}
      ]
    })
    .compileComponents();
    sucursalIdServiceMock.getId.and.returnValue(of(mockSucursalId))
    BebidasCalientesStockServiceMock.getBebidasCalientes.and.returnValue(of(mockBebidas))
    fixture = TestBed.createComponent(BebidasCalientesStockComponent);
  ...
  it('should show alert and navigate to admin when token expired error occurs', async () => {
    BebidasCalientesStockServiceMock.getBebidasCalientes.and.returnValue(throwError({ error: { message: 'Token expired' } }));
  ...
  it('should update the existence', async () => {
    ...
    expect(BebidasCalientesStockServiceMock.updateStockBebidaCaliente).toHaveBeenCalledWith(mockBebidasExistencia[1].id,{existencia: mockBebidasExistencia[1].existencia, cantidad: mockBebidasExistencia[1].cantidad, sucursal_id: component.bebida1.sucursal_id});
    await BebidasCalientesStockServiceMock.updateStockBebidaCaliente.and.returnValue(of(mockSucursalId))
     expect(BebidasCalientesStockServiceMock.getBebidasCalientes).toHaveBeenCalledWith(component.bebida1)
  });
  ...
  it('should update the inventory', async () => {
    ...
    expect(BebidasCalientesStockServiceMock.updateStockBebidaCaliente).toHaveBeenCalledWith(mockBebidasInventario[0].id,{existencia: mockBebidasInventario[0].existencia, cantidad: mockBebidasInventario[0].cantidad, inventario: mockBebidasInventario[0].inventario, sucursal_id: component.bebida1.sucursal_id});
    await BebidasCalientesStockServiceMock.updateStockBebidaCaliente.and.returnValue(of(mockSucursalId))
     expect(BebidasCalientesStockServiceMock.getBebidasCalientes).toHaveBeenCalledWith(component.bebida1)
  });
});
