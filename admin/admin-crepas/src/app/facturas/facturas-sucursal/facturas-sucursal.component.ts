...
@Component({
  selector: 'app-facturas-sucursal',
  templateUrl: './facturas-sucursal.component.html',
  styleUrls: ['./facturas-sucursal.component.css']
})
export class FacturasSucursalComponent {
  ...
  authService: any = {};
  constructor(private bebidasCalientesSalesService: FacturasService, private idService: IdService,private alert: AlertDialogService, private router: Router, private alertService: AlertDialogService, AuthService: AuthService) {
  ...
  ngOnInit(): void {
    ...
    this.bebidasCalientesSalesService.getInvoices(this.bebida1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.fecha_hora;
...
res[index].fecha_hora = fechaFormateada;
        })
        this.facturas = res
  ...
  genarar(){
    ...
    this.bebidasCalientesSalesService.lapsInvoice(this.bebida1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.fecha_hora;
          // Separa la fecha en partes
...
res[index].fecha_hora = fechaFormateada;
  ...
  viewUsuario(id:any){
    this.router.navigate(['/viewUsuarioCaja/'+ id]);
  }
