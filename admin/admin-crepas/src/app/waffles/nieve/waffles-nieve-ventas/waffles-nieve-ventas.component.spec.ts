...
import { WafflesNieveVentasComponent } from './waffles-nieve-ventas.component';
...
describe('WafflesNieveVentasComponent', () => {
  let component: WafflesNieveVentasComponent;
  let fixture: ComponentFixture<WafflesNieveVentasComponent>;
  let alertService: jasmine.SpyObj<AlertDialogService>;
  let waffleSalesService: jasmine.SpyObj<WaffleSalesService>;
  const mockRouter = jasmine.createSpyObj('Router', ['navigate']);
  const mockNieveSales = [
    {
      nieve: "Café Americano",
      product_id: 1,
      sucursal_id: 1,
      factura_id: 1234,
      ventas: 10,
  ...
  const mockNieveSalesLaps = [
    // ... Bebidas del primer set (10) ...
  
    {
      nieve: "Té de Manzanilla",
      product_id: 11,
      sucursal_id: 1,
      factura_id: 1244,
      ventas: 2,
  ...
  let NieveSalesServiceMock = {
    getNieves: jasmine.createSpy('getNieves').and.returnValue(of(mockNieveSales)),
    lapsSalesNieves: jasmine.createSpy('lapsSalesNieves').and.returnValue(of(mockNieveSalesLaps))
  }
  let AuthServicMock = {
  ...
  beforeEach(async () => {
    ...
    waffleSalesService = jasmine.createSpyObj('WaffleSalesService', ['getNieves', 'lapsSalesNieves']);
    await TestBed.configureTestingModule({
      imports: [ HttpClientTestingModule, FormsModule],
      declarations: [ WafflesNieveVentasComponent, TranslatePipe ],
      providers: [
        { provide: AlertDialogService, useValue: alertService },
        { provide: Router, useValue: mockRouter },
        { provide: WaffleSalesService, useValue: NieveSalesServiceMock},
        { provide: AuthService, useValue: AuthServicMock},
      ]
    })
    .compileComponents();

    fixture = TestBed.createComponent(WafflesNieveVentasComponent);
    component = fixture.componentInstance;
    component.sucursal_id = 1;
    component.nieve1.sucursal_id = 1;
    component.nieve1.fecha = new Date();
  ...
  it('should give the formatted date', async () => {
    ...
    expect(NieveSalesServiceMock.getNieves).toHaveBeenCalledWith({fecha:date1, sucursal_id: 1})

    NieveSalesServiceMock.getNieves.and.returnValue(of(mockNieveSales))
  });
  ...
  it('should generate sales', async () => {
    ...
    nieve1.fechaInicio = fecha.fecha1;
    nieve1.fechaFin = fecha.fecha2;
    nieve1.sucursal_id = 1;
    nieve1.fecha = date1

    
    component.genarar();
    await fixture.whenStable();
    expect(NieveSalesServiceMock.lapsSalesNieves).toHaveBeenCalledWith(nieve1)

    NieveSalesServiceMock.lapsSalesNieves.and.returnValue(of(mockNieveSalesLaps))
  });
  ...
  it('should show alert and navigate to admin when token expired error occurs', async () => {
    AuthServicMock.lang.and.returnValue('en')
    component.nieve1.fecha = new Date()
    NieveSalesServiceMock.getNieves.and.returnValue(throwError({ error: { message: 'Token expired' } }));
