...
import { BebidasFriasSalesGlobalService } from 'src/app/services/salesGlobal/bebidas-frias-sales.service';

@Component({
  selector: 'app-bebidas-frias-ventas',
  templateUrl: './bebidas-frias-ventas.component.html',
  styleUrls: ['./bebidas-frias-ventas.component.css']
})
export class BebidasFriasVentasGlobalesComponent {
  ...
  constructor(private bebidasFriasSalesService: BebidasFriasSalesGlobalService, private idService: IdService,private alert: AlertDialogService,private router: Router, private authService: AuthService, private alertService: AlertDialogService) {}
  ...
  ngOnInit(): void {
    ...
    this.bebidasFriasSalesService.getBebidasFrias(this.bebida1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.bebidasFriasSalesService.lapsSaleBebidaFria(this.bebida1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
