...
import { CrepaDulceSalesGlobalService } from 'src/app/services/salesGlobal/crepa-dulce-sales.service';

@Component({
  selector: 'app-crepa-dulce-harinas-ventas',
  templateUrl: './crepa-dulce-harinas-ventas.component.html',
  styleUrls: ['./crepa-dulce-harinas-ventas.component.css']
})
export class CrepaDulceHarinasVentasGlobalesComponent {
  ...
  constructor(private service: CrepaDulceSalesGlobalService, private idService: IdService,private alert: AlertDialogService,private router: Router, private authService: AuthService) {}
  ...
  ngOnInit(): void {
    this.harina1.fecha = this.formatDate(this.harina1.fecha);


    this.service.getHarinas(this.harina1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.harina1.fechaInicio = fecha.fecha1;
    this.harina1.fechaFin = fecha.fecha2;
    this.service.lapsSalesHarinas(this.harina1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
