...
import { WaffleCanastaSalesGlobalService } from 'src/app/services/salesGlobal/waffle-canasta-sales.service';
@Component({
  selector: 'app-waffle-canasta-ingrediente-untable-ventas',
  templateUrl: './waffle-canasta-ingrediente-untable-ventas.component.html',
  styleUrls: ['./waffle-canasta-ingrediente-untable-ventas.component.css']
})
export class WaffleCanastaIngredienteUntableVentasGlobalesComponent {
  ...
  constructor(private service: WaffleCanastaSalesGlobalService, private idService: IdService,private alert: AlertDialogService,private router: Router, private authService: AuthService) {}
  ...
  ngOnInit(): void {
    ...
    this.service.getIngredientesUntables(this.ingrediente1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.service.lapsSalesIngredientesUntables(this.ingrediente1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
