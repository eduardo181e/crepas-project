...
@Component({
  selector: 'app-crepa-salada-ingrediente-principal-ventas',
  templateUrl: './crepa-salada-ingrediente-principal-ventas.component.html',
  styleUrls: ['./crepa-salada-ingrediente-principal-ventas.component.css']
})
export class CrepaSaladaIngredientePrincipalVentasGlobalesComponent {
  ...
  ngOnInit(): void {
    ...
    this.service.getIngredientesPrincipal(this.ingrediente1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.service.lapsSalesIngredientesPrincipal(this.ingrediente1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
