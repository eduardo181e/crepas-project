...
@Component({
  selector: 'app-crepa-salada-botana-ventas',
  templateUrl: './crepa-salada-botana-ventas.component.html',
  styleUrls: ['./crepa-salada-botana-ventas.component.css']
})
export class CrepaSaladaBotanaVentasGlobalesComponent {
  ...
  ngOnInit(): void {
    this.botana1.fecha = this.formatDate(this.botana1.fecha);


    this.service.getBotanas(this.botana1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.botana1.fechaInicio = fecha.fecha1;
    this.botana1.fechaFin = fecha.fecha2;
    this.service.lapsSalesBotanas(this.botana1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
