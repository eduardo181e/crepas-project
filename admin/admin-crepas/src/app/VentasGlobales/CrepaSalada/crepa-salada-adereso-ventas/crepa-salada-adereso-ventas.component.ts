...
@Component({
  selector: 'app-crepa-salada-adereso-ventas',
  templateUrl: './crepa-salada-adereso-ventas.component.html',
  styleUrls: ['./crepa-salada-adereso-ventas.component.css']
})
export class CrepaSaladaAderesoVentasGlobalesComponent {
  ...
  ngOnInit(): void {
    ...
    this.service.getAderesos(this.adereso1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.service.lapsSalesAdereso(this.adereso1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
