...
@Component({
  selector: 'app-crepa-salada-ingrediente-base-ventas',
  templateUrl: './crepa-salada-ingrediente-base-ventas.component.html',
  styleUrls: ['./crepa-salada-ingrediente-base-ventas.component.css']
})
export class CrepaSaladaIngredienteBaseVentasGlobalesComponent {
  ...
  ngOnInit(): void {
    ...
    this.service.getIngredientesBase(this.ingrediente1).subscribe(
...
res[index].created_at = fechaFormateada;
  ...
  genarar(){
    ...
    this.service.lapsSalesIngredientesBase(this.ingrediente1).subscribe(
      (res:any) => {console.log(res)
        res.forEach((element:any , index:any) => {
          const fecha = element.created_at;
          // Separa la fecha en partes
