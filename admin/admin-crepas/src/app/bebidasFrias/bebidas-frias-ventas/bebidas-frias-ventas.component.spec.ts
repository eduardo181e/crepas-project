...
import { BebidasFriasVentasComponent } from './bebidas-frias-ventas.component';
...
import { BebidasFriasSalesService } from 'src/app/services/sales/bebidas-frias-sales.service';
import { AuthService } from 'src/app/services/auth-service.service';

describe('BebidasFriasVentasComponent', () => {
  let component: BebidasFriasVentasComponent;
  let fixture: ComponentFixture<BebidasFriasVentasComponent>;
  let alertService: jasmine.SpyObj<AlertDialogService>;
  let bebidasFriasSalesService: jasmine.SpyObj<BebidasFriasSalesService>;
  const mockRouter = jasmine.createSpyObj('Router', ['navigate']);
  const mockBebidasFriasSales = [
    {
  ...
  const mockBebidasFriasSalesLaps = [
  ...
  let BebidasFriasSalesServiceMock = {
    getBebidasFrias: jasmine.createSpy('getBebidasFrias').and.returnValue(of(mockBebidasFriasSales)),
    lapsSaleBebidaFria: jasmine.createSpy('updateStockBebidaCaliente').and.returnValue(of(mockBebidasFriasSalesLaps))
  }
  let AuthServicMock = {
  ...
  beforeEach(async () => {
    ...
    bebidasFriasSalesService = jasmine.createSpyObj('BebidasFriasSalesServiceMock', ['getBebidasFrias']);
    await TestBed.configureTestingModule({
      imports: [ HttpClientTestingModule, FormsModule],
      declarations: [ BebidasFriasVentasComponent, TranslatePipe ],
      providers: [
        { provide: AlertDialogService, useValue: alertService },
        { provide: Router, useValue: mockRouter },
        { provide: BebidasFriasSalesService, useValue: BebidasFriasSalesServiceMock},
        { provide: AuthService, useValue: AuthServicMock}]
    })
    .compileComponents();

    fixture = TestBed.createComponent(BebidasFriasVentasComponent);
  ...
  it('should give the formatted date', async () => {
    ...
    expect(BebidasFriasSalesServiceMock.getBebidasFrias).toHaveBeenCalledWith({fecha:date1, sucursal_id: 1})

    BebidasFriasSalesServiceMock.getBebidasFrias.and.returnValue(of(mockBebidasFriasSales))
  });
  ...
  it('should generate sales', async () => {
    ...
    component.genarar();
    await fixture.whenStable();
    expect(BebidasFriasSalesServiceMock.lapsSaleBebidaFria).toHaveBeenCalledWith(bebida1)

    BebidasFriasSalesServiceMock.lapsSaleBebidaFria.and.returnValue(of(mockBebidasFriasSalesLaps))
  });
  ...
  it('should show alert and navigate to admin when token expired error occurs', async () => {
    AuthServicMock.lang.and.returnValue('en')
    component.bebida1.fecha = new Date()
    BebidasFriasSalesServiceMock.getBebidasFrias.and.returnValue(throwError({ error: { message: 'Token expired' } }));
