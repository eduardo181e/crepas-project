...
import { CrepaSaladaAderesoVentasComponent } from './crepa-salada-adereso-ventas.component';
...
describe('CrepaSaladaAderesoVentasComponent', () => {
  let component: CrepaSaladaAderesoVentasComponent;
  let fixture: ComponentFixture<CrepaSaladaAderesoVentasComponent>;
  let alertService: jasmine.SpyObj<AlertDialogService>;
  let crepaSaladaSalesService: jasmine.SpyObj<CrepaSaladaSalesService>;
  const mockRouter = jasmine.createSpyObj('Router', ['navigate']);
  const mockaderesoSales = [
    {
      adereso: "Café Americano",
      product_id: 1,
      sucursal_id: 1,
      factura_id: 1234,
      ventas: 10,
  ...
  const mockaderesoSalesLaps = [
    // ... Bebidas del primer set (10) ...
  
    {
      adereso: "Té de Manzanilla",
      product_id: 11,
      sucursal_id: 1,
      factura_id: 1244,
      ventas: 2,
  ...
  let aderesoSalesServiceMock = {
    getAderesos: jasmine.createSpy('getAderesos').and.returnValue(of(mockaderesoSales)),
    lapsSalesAdereso: jasmine.createSpy('lapsSalesAdereso').and.returnValue(of(mockaderesoSalesLaps))
  }
  let AuthServicMock = {
  ...
  beforeEach(async () => {
    ...
    crepaSaladaSalesService = jasmine.createSpyObj('CrepaSaladaSalesService', ['getAderesos', 'lapsSalesAdereso']);
    await TestBed.configureTestingModule({
      imports: [ HttpClientTestingModule, FormsModule],
      declarations: [ CrepaSaladaAderesoVentasComponent, TranslatePipe ],
      providers: [
        { provide: AlertDialogService, useValue: alertService },
        { provide: Router, useValue: mockRouter },
        { provide: CrepaSaladaSalesService, useValue: aderesoSalesServiceMock},
        { provide: AuthService, useValue: AuthServicMock},
      ]
    })
    .compileComponents();

    fixture = TestBed.createComponent(CrepaSaladaAderesoVentasComponent);
  ...
  it('should give the formatted date', async () => {
    ...
    expect(aderesoSalesServiceMock.getAderesos).toHaveBeenCalledWith({fecha:date1, sucursal_id: 1})

    aderesoSalesServiceMock.getAderesos.and.returnValue(of(mockaderesoSales))
  });
  ...
  it('should generate sales', async () => {
    ...
    component.genarar();
    await fixture.whenStable();
    expect(aderesoSalesServiceMock.lapsSalesAdereso).toHaveBeenCalledWith(adereso1)

    aderesoSalesServiceMock.lapsSalesAdereso.and.returnValue(of(mockaderesoSalesLaps))
  });
  ...
  it('should show alert and navigate to admin when token expired error occurs', async () => {
    AuthServicMock.lang.and.returnValue('en')
    component.adereso1.fecha = new Date()
    aderesoSalesServiceMock.getAderesos.and.returnValue(throwError({ error: { message: 'Token expired' } }));
