...
import { IngredientesComplementariosWaffleCanastaComponent } from './ingredientes-complementarios.component';
...
describe('IngredientesComplementariosWaffleCanastaComponent', () => {
  let component: IngredientesComplementariosWaffleCanastaComponent;
  let fixture: ComponentFixture<IngredientesComplementariosWaffleCanastaComponent>;
  let alertService: jasmine.SpyObj<AlertDialogService>;
  let waffleCanastaService: jasmine.SpyObj<WaffleCanastaService>;
  const mockRouter = jasmine.createSpyObj('Router', ['navigate']);
  const mockIngredientes = [{ ingrediente_com: 'Vainilla',inventario: '1', id: '2', tipo: "Frutos Secos" },{ ingrediente_com: 'Fresa', inventario: '2', id: '4', tipo: "Otros"} ];
  let ingredinetesMock = {
    getIngredientesC: jasmine.createSpy('getIngredientesC').and.returnValue(of(mockIngredientes)),
    deleteIngredienteC: jasmine.createSpy('deleteIngredienteC').and.returnValue(of()),
  }
  let AuthServicMock = {
  ...
  beforeEach(async () => {
    ...
    waffleCanastaService = jasmine.createSpyObj('WaffleCanastaService', ['getIngredientesC', 'deleteIngredienteC']);
    await TestBed.configureTestingModule({
      imports: [ FormsModule, HttpClientTestingModule ],
      declarations: [ IngredientesComplementariosWaffleCanastaComponent, TranslatePipe ],
      providers: [
    ...
    fixture = TestBed.createComponent(IngredientesComplementariosWaffleCanastaComponent);
  ...
  it('should show the drinks', async () => {
    ingredinetesMock.getIngredientesC.and.returnValue(of(mockIngredientes))
    component.ngOnInit()
  ...
  it('debería mostrar una alerta y navegar hasta el administrador cuando se produzca un error de caducidad del token', async () => {
    ...
    ingredinetesMock.getIngredientesC.and.returnValue(throwError({ error: { message: 'Token expired' } }));
  ...
  it('should return an object without the object with the id that was deleted', async () => {
    ...
    ingredinetesMock.deleteIngredienteC.and.returnValue(of())
  ...
  it('should return an object without the object with the id that was deleted (error token expired)', async () => {
    AuthServicMock.lang.and.returnValue('en')
    ingredinetesMock.deleteIngredienteC.and.returnValue(throwError({ error: { message: 'Token expired' } }));
    component.deleteIngrediente(4)
    await fixture.whenStable();
    expect(alertService.mostrarAlerta).toHaveBeenCalledWith('Your session has expired, please log in again');
    expect(mockRouter.navigate).toHaveBeenCalledWith(['admin']);
  ...
  it('should navigate to create', () => {
    ...
    expect(mockRouter.navigate).toHaveBeenCalledWith(['createWafflesCanastaIngredienteComplementario']);
  });

  it('should navigate to edit', () => {
    component.navigateEdit(2)
    expect(mockRouter.navigate).toHaveBeenCalledWith(['editWafflesCanastaIngredienteComplementario', 2]);
  });
});
