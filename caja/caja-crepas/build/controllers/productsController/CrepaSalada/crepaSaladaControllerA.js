...
class CrepaSaladaAderesoController {
    list(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const aderesos = yield database_1.default.promise().query(query, [sucursal, adminId])
                    .then(aderesos => aderesos[0])
                    .catch(err => console.log(err));
                const aderesos2 = yield database_1.default.promise().query('SELECT * FROM csa WHERE adminId = ?', [adminId])
                    .then(aderesos2 => aderesos2[0])
                    .catch(err => console.log(err));
                const existenciasId = {};
                aderesos.forEach((existencia) => {
                    existenciasId[existencia.product_id] = existencia;
                });
                const resultado = aderesos2.map((adereso) => {
    ...
    getOne(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const aderesos = yield database_1.default.promise().query(query, [aderesoId])
                .then(aderesos => aderesos[0])
                .catch(err => console.log(err));
